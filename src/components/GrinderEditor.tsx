...
interface GrinderEditorProps {
    onClose: () => void;
    initialView?: 'list' | 'form';
}

export const GrinderEditor: React.FC<GrinderEditorProps> = ({ onClose, initialView = 'list' }) => {
    ...
    const [scaleType, setScaleType] = useState<'linear' | 'eureka'>('linear');
    const [images, setImages] = useState<string[]>([]);
    const [thumbnails, setThumbnails] = useState<string[]>([]);

    const SETTING_KEY = 'grinders_list';
    ...
    const handleOpenEdit = (item: ListItem) => { 
        ...
        setScaleType(item.scaleType || 'linear');
    ...
    const handleDeleteItem = async (id: string) => {
        if (!confirm("Ștergi această râșniță?")) return;
                            ...
                            {items.slice(0, displayLimit).map((item, index) => (
                                <div key={item.id} className="flex flex-col p-4 rounded-2xl border border-white/5 bg-surface-container shadow-sm active:scale-[0.99] transition-all">
                                    ...
                                    <div className="flex flex-col items-center text-center gap-1 mb-4">
                                        ...
                                        <span className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider bg-surface-container-high px-2 py-0.5 rounded mt-1 border border-white/5">
                                            {item.scaleType === 'eureka' ? 'SCALĂ CADRAN (EUREKA)' : 'SCALĂ LINIARĂ (CLASIC)'}
                                        </span>
                                    </div>
                ...
                <div className="flex flex-col gap-4 animate-fade-in pb-20 overflow-y-auto no-scrollbar pt-10">
                    <div className="bg-surface-container rounded-2xl p-5 border border-white/10 shadow-md">
                        ...
                        <div className="relative">
                            <input value={inputValue} onChange={(e) => setInputValue(e.target.value)} className={`${INPUT_STYLE} h-14 text-lg bg-surface`} placeholder="Ex: Niche Zero..." />
                        </div>
                    </div>

                    {/* Scale Type Selector */}
                    <div className="bg-surface-container rounded-2xl p-4 border border-white/10 shadow-md">
                        <label className={LABEL_STYLE}>Tip Scală (UI)</label>
                        <div className="grid grid-cols-2 gap-3 mt-2">
                            ...
                            >
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-wider">Liniară (Generic)</span>
                            </button>
                            <button 
                                onClick={() => setScaleType('eureka')}
                                className={`h-16 rounded-xl border flex flex-col items-center justify-center gap-1 transition-all ${
                                    scaleType === 'eureka' 
                            ...
                            >
                                <div className="w-5 h-5 rounded-full border-2 border-current flex items-center justify-center text-[8px] font-bold">12</div>
                                <span className="text-[10px] font-bold uppercase tracking-wider">Cadran (Eureka)</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className={`${BOX_STYLE} h-auto min-h-[140px] flex flex-col justify-between`}>
                       <span className={LABEL_STYLE}>FOTO RÂȘNIȚĂ</span>
                         ...
                         {images.length === 0 ? <CameraIcon className="w-8 h-8 text-on-surface-variant opacity-30 drop-shadow-sm" /> : (
                            images.map((img, idx) => (
                                <div key={idx} className="relative w-16 h-16 shrink-0 rounded-xl overflow-hidden shadow-md border border-white/10">
                                   <img src={thumbnails[idx] || img} className="w-full h-full object-cover" alt={`Grinder ${idx}`} />
                    ...
                    <div><label className={LABEL_STYLE}>DESCRIERE</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} className={`${INPUT_STYLE} h-32 resize-none leading-relaxed text-left`} placeholder="Expertul va completa aici..." /></div>
                    ...
                    <div className="bg-surface-container rounded-[2rem] w-full max-w-sm p-6 shadow-2xl flex flex-col gap-4 max-h-[85vh] border border-white/10">
                        ...
                        <div className="overflow-y-auto space-y-4 pr-1 no-scrollbar pb-4">
                            ...
                            <div className="bg-surface-container-high rounded-xl p-3 border border-white/5 text-center">
                                <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest block mb-1">Tip Scală</span>
                                <span className="text-sm font-bold text-on-surface">{viewingItem.scaleType === 'eureka' ? 'Cadran (Eureka)' : 'Liniară (Generic)'}</span>
                            </div>
