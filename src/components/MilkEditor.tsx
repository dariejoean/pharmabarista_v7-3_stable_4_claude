...
interface MilkEditorProps {
    onClose: () => void;
    initialView?: 'list' | 'form';
}

export const MilkEditor: React.FC<MilkEditorProps> = ({ onClose, initialView = 'list' }) => {
    ...
    const handleDeleteItem = async (id: string) => {
        if (!confirm("Ștergi acest tip de lapte?")) return;
                ...
                <div className="flex flex-col gap-4 animate-fade-in pb-20 overflow-y-auto no-scrollbar pt-10">
                    <div className="bg-surface-container rounded-2xl p-5 border border-white/10 shadow-md">
                        ...
                        <div className="relative">
                            <input value={inputValue} onChange={(e) => setInputValue(e.target.value)} className={`${INPUT_STYLE} h-14 text-lg bg-surface`} placeholder="Ex: Lapte Ovăz Barista..." />
                        </div>
                    </div>
                    {/* Photo Upload Block */}
                    <div className={`${BOX_STYLE} h-auto min-h-[140px] flex flex-col justify-between`}>
                       <span className={LABEL_STYLE}>FOTO LAPTE</span>
                         ...
                         {images.length === 0 ? <CameraIcon className="w-8 h-8 text-on-surface-variant opacity-30 drop-shadow-sm" /> : (
                            images.map((img, idx) => (
                                <div key={idx} className="relative w-16 h-16 shrink-0 rounded-xl overflow-hidden shadow-md border border-white/10">
                                   <img src={thumbnails[idx] || img} className="w-full h-full object-cover" alt={`Milk ${idx}`} />
